...
import { buildWeakAreaIdentifierPrompt } from "@/lib/prompts/weakAreaIdentifier";
import {
  parseWeakAreaIdentifierJson,
  validateWeakAreaIdentifierRequest,
} from "@/lib/validators/weakAreaIdentifier";
...
export async function POST(request: Request) {
  try {
    ...
    const validated = validateWeakAreaIdentifierRequest(body);
    if (!validated.ok) {
      return NextResponse.json({ error: validated.error }, { status: 400 });
    }

    const prompt = buildWeakAreaIdentifierPrompt(validated.data);
    ...
    const parsed = parseWeakAreaIdentifierJson(extractedJson ?? text);
