...
import { buildCurrentAffairsDigestPrompt } from "@/lib/prompts/currentAffairsDigest";
import {
  parseCurrentAffairsDigestJson,
  validateCurrentAffairsDigestRequest,
} from "@/lib/validators/currentAffairsDigest";
...
export async function POST(request: Request) {
  try {
    ...
    const validated = validateCurrentAffairsDigestRequest(body);
    if (!validated.ok) {
      return NextResponse.json({ error: validated.error }, { status: 400 });
    }

    const prompt = buildCurrentAffairsDigestPrompt(validated.data);
    ...
    if (!completionResponse.ok) {
      ...
      return NextResponse.json(
        { error: "Failed to generate current affairs digest.", details: apiErrorMessage },
        { status: 502 },
      );
    }
    ...
    const parsed = parseCurrentAffairsDigestJson(extractedJson ?? text);
    ...
    return NextResponse.json(
      { error: "Failed to generate current affairs digest.", details: message },
      { status: 500 },
    );
  }
}
